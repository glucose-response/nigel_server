    ...
    for sheet_name in (sheet_names):
        ...
        try: 
            ...
            if 'NigelID' in df.columns and 'Timestamp' in df.columns:
                ...
                grouped_data = df.groupby(['NigelID', df['Timestamp'].dt.date])

                for (NigelID, day), entries in grouped_data:
                    # Find the existing document with the given 'NigelID'
                    existing_document = db[sheet_name].find_one({'NigelID': int(NigelID)})
                    ...
                    if existing_document:
                        ...
                        existing_entries[day.strftime('%Y-%m-%d')] = entries.to_dict(orient='records')
                        ...
                        db[sheet_name].insert_one({'NigelID': int(NigelID), 'entries': {day.strftime('%Y-%m-%d'): entries.to_dict(orient='records')}})
